...
#define DELAY_TO_DEAL_NEXT_HOLE_CARD 0.5
#define DELAY_TO_DEAL_FLOP 7.0
#define DELAY_TO_DEAL_TURN_RIVER 7.0
#define SHOWDOWN_DELAY 9.0
#define INFO_DELAY 0.002
#define DELAY_DEAL_TO_THE_RIVER 7.0

#define SIMULATION_ITERATIONS 300
...
@implementation TrainingModeView
...
- (void) resetStuff {
	...
	applicationData = malloc(HOLDEM_TRAINING_MODE_APPLICATION_DATA_LENGTH * sizeof(uint8_t));
...
- (void) savePercentagesInApplicationData {
	applicationData[14] = preflopHeroPercentage;
	applicationData[15] = preflopVillainPercentage;
	applicationData[16] = flopHeroPercentage;
	applicationData[17] = flopVillainPercentage;
}

- (NSData*) getApplicationData {
	return [[[NSData alloc] initWithBytes:applicationData length:HOLDEM_TRAINING_MODE_APPLICATION_DATA_LENGTH] autorelease];
}

- (void) clearApplicationData {
	applicationData[0] = (uint8_t)0;
	[appController writeHoldemTrainingModeApplicationData:[self getApplicationData]];
}
...
- (void) highlightHand:(BOOL)isHeroHand {
	...
	if (hand.street == kStreetRiver || [self isSameCardsMode]) {
		...
		[bestHand.cards addObject:communityCard1View.card];
		[bestHand.cards addObject:communityCard2View.card];
...
- (void) willRestoreFromApplicationData:(uint8_t*)appData {
	...
	memcpy(applicationData, appData, HOLDEM_TRAINING_MODE_APPLICATION_DATA_LENGTH);
	...
	// restore 9 cards
	for (int cardIndex = 5; cardIndex <= 13; cardIndex++) {
	...
	preflopHeroPercentage = applicationData[14];
	preflopVillainPercentage = applicationData[15];
	flopHeroPercentage = applicationData[16];
	flopVillainPercentage = applicationData[17];
