//
//  NoLimitHoldemVillainCS.m
//  A.I. engine that models a "calling station" player, i.e. one who calls too much.
//  Headsup
//
...
#import "NoLimitHoldemVillainLAG.h"
#import "Card.h"
#import "CardView.h"
#import "Deck.h"
#import "Hand.h"
...
@implementation NoLimitHoldemVillainLAG
...
- (BOOL) isRiverHandPairPlus:(MadeHand*)hand {
	return 
	hand.type == kTypeNonTopPair ||
	hand.type == kTypeTopPair ||
	hand.type == kTypeTwoPair ||
	hand.type == kTypeTrips ||
...
- (BOOL) isRiverHandStrong:(MadeHand*)hand {
	return 
	hand.type == kTypeTrips ||
	hand.type == kTypeStraight ||
	hand.type == kTypeFlush ||
...
- (Move*) bet:(NSInteger)betPercentage
  callNearPot:(NSInteger)callPercentage {
	...
	if ([gameModeView didHeroMakeNoMove] || [gameModeView didHeroCheck]) {
		...
		if (callAmount > 0 && callAmount <= pot * 2 / 3) {
...
- (Move*) bluff:(NSInteger)betPercentage 
	...
	if ([gameModeView didHeroMakeNoMove] || [gameModeView didHeroCheck]) {
		...
		if ([self isRand:callPercentage]) {
			if ([self isRand:30])
				move = [[Move alloc] initWithMove:kMoveRaise Amount:[gameModeView halfPotSizedRaiseForVillain]];
			else
				move = [[Move alloc] initWithMove:kMoveCall Amount:0];
		} else {				
			move = [[Move alloc] initWithMove:kMoveFold Amount:0];
...
- (Move*) riverMove {
		...
		if ([self isRiverHandWeak:hand0]) {
			...
			if ([self isRiverHandWeak:hand1]) {
				...
				if (hand1.type == kTypeHighCard ||
					...
					// villain has top pair
					move = [self bet:30 callNearPot:80];
				}
			} else {
				// villain has true two pair or better
				if ([self isRiverHandSuperStrong:hand1]) {
					...
					// villain's hand is not super strong. but he won't fold
					// because he's a calling station.
					move = [self bet:25 goNuts:NO willFold:NO];
				}
			}
		}
	...
	if (move.move == kMoveFold) {
		if (hand0.type == kTypeHighCard || hand0.type == kTypeNonTopPair) {
			// hero has a weak hand. why should villain fold?
			if (callAmount <= pot && [self isRand:60] && [self isRiverHandPairPlus:hand1])
				move.move = kMoveCall;
		} else if (result == NSOrderedAscending || result == NSOrderedSame) {
			// hero doesn't have a better hand. why should villain fold?
			if (callAmount <= pot && [self isRand:80] && [self isRiverHandPairPlus:hand1])
				move.move = kMoveCall;
		} else if (hand0.type == kTypeTwoPair) {
			// hero has two pair. we want to call him to make him happy. 
			if (callAmount <= pot && [self isRand:30])
				move.move = kMoveCall;			
		} else if (hand0.type == kTypeTopPair || hand0.type == kTypeOverPair) {
			// hero has top/over pair. we want to call him to make him happy. 
			if (callAmount <= pot && [self isRand:30])
				move.move = kMoveCall;						
		} else if ([self isRiverHandStrong:hand0]) {
			// hero has a good hand. we want to call him to make him happy. 
			if (callAmount <= pot && [self isRand:70] && [self isRiverHandPairPlus:hand1])
				move.move = kMoveCall;
		}
	} else if (move.move == kMoveCheck) {
		if ([self isRiverHandStrong:hand1]) {
			if ([gameModeView didHeroPostBB]) {
				// villain is dealer, meaning villain is checked to. always value bet 
				// in this spot.
				move.move = kMoveBet;
				move.amount = [gameModeView halfPotSizedBetForVillain];
			}
		}
	} else if (move.move == kMoveCall) {
		// call needs to be reevaluated if hero's bet is bigger than pot.
		// basically if villain has a worse hand and the hand is weak, villain ought to
		// fold rather than call facing an overbet. On the other hand, villain ought not
		// to call if he has a better yet weak hand if hero bets too much.
		if (callAmount > pot) {
			if (result == NSOrderedDescending && ![self isRiverHandStrong:hand1]) {
				move.move = kMoveFold;
			} 
			
			if (callAmount > 2 * pot) {
				if (result == NSOrderedAscending && [self isRiverHandWeak:hand1]) {
					move.move = kMoveFold;
				}
			}
		}
...
- (Move*) turnMove {
		...
		if ([self isMonsterDraw:hand1]) {
			...
			if ([self isDecentDraw:hand1]) {
				...
				if ([self isRiverHandWeak:hand0]) {
					...
					if (hand0.type == kTypeHighCard ||
						hand0.type == kTypeNonTopPair)
						move = [self bluff:50 call:50];
					else
						move = [self bluff:30 call:30];			
				} else {
					// hero has true two pair or better
					if ([self isRiverHandWeak:hand1]) {
						// villain has worse than true two pair
						if (hand1.type == kTypeHighCard ||
							hand1.type == kTypeNonTopPair) {
							move = [self bluff:20 call:20];
						} else {
							move = [self bet:30 callNearPot:60];
						}
					} else {
						// villain has true two pair or better
						if ([self isRiverHandSuperStrong:hand1]) {
	...
	if (move.move == kMoveFold) {
		...
		if (hand1.fd || hand1.oesd || hand1.dgsd) {
			...
			// hero has a weak hand. why should villain fold?
			if (callAmount <= pot && [self isRand:60])
				move.move = kMoveCall;
		} else if (result == NSOrderedAscending || result == NSOrderedSame) {
			// hero doesn't have a better hand. why should villain fold?
			if (callAmount <= pot && [self isRand:80])
				move.move = kMoveCall;
		} else if (hand0.type == kTypeTwoPair) {
			// hero has two pair. we want to call him to make him happy. 
			if (callAmount <= pot && [self isRand:40])
				move.move = kMoveCall;			
		} else if (hand0.type == kTypeTopPair || hand0.type == kTypeOverPair) {
			// hero has top/over pair. we want to call him to make him happy. 
			if (callAmount <= pot && [self isRand:40])
			...
			// hero has a good hand. we want to call him to make him happy. 
			if (callAmount <= pot && [self isRand:70])
				move.move = kMoveCall;
		}
	} else if (move.move == kMoveCall) {
		// call needs to be reevaluated if hero's bet is bigger than pot.
				...
				if ([self isRiverHandWeak:hand0]) {
					...
					if ([self isRiverHandWeak:hand1]) {
						...
						if (hand1.type == kTypeHighCard ||
							...
							move = [self bet:40 callNearPot:50];
						}
					} else {
						// villain has true two pair or better
						if ([self isRiverHandSuperStrong:hand1]) {
							...
							// villain's hand is not super strong. he could find a fold.
							move = [self bet:30 goNuts:NO willFold:NO];
						}
					}
				}
			}
...
- (Move*) preflopMove {
	...
	if (move.move == kMoveFold) {		
		...
		// call needs to be reevaluated if hero's bet is bigger than pot.
		// basically if villain has a worse hand and the hand is weak, villain ought to
		// fold rather than call facing an overbet.
		if (callAmount > pot) {
			if ([Hand calcWinnerHoleCard0:c2 holeCard1:c3 holeCard2:c0 holeCard3:c1] == NSOrderedDescending 
				&& ![Hand isTier1:c0 :c1]) {
				move.move = kMoveFold;
			}			
		}
	...
	if (move.move == kMoveFold && 
		...
		move = [self antiBluffRaise:20 call:40];
	}
