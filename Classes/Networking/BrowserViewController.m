...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (((AppController*)[[UIApplication sharedApplication] delegate]).errorOccurred) {
		isResolving = NO;
	}
	
	if (!isResolving) {
		isResolving = YES;
		((AppController*)[[UIApplication sharedApplication] delegate]).errorOccurred = NO;
...
{
	NSLog(@"adwhirl key browser");
	return @"d5141bb89578102caf90c29cca1d33aa";
}

- (UIViewController *)viewControllerForPresentingModalView {
	NSLog(@"viewcontrollerforpresentingmodalview browser");
...
- (NSString *)googleAdSenseCompanyName {
	NSLog(@"google adsense company name browser");
